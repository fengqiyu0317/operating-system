...
fn get_app_name(app_id: usize) -> &'static str {
    unsafe {
        let mut name_ptr = _app_names as *const () as usize;
        // 跳过前面的 app_id 个字符串
        for _ in 0..app_id {
...
lazy_static! {
    ...
    /// 应用用户态/内核态时间统计
    static ref APP_TIME_STATS: UPSafeCell<[AppTimeStats; MAX_APP_NUM]> = unsafe {
        UPSafeCell::new([AppTimeStats::new(); MAX_APP_NUM])
    };
}
